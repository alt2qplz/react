...
let AddNewMessage = (props) => {
  return (
    <form onSubmit={props.handleSubmit} className={`${s.newMessageForm} white-container`}>
      <Field name='newMessageBody' placeholder='Введите сообщение' component='textarea' type='text'
...
const Messages = (props) => {
  ...
  let messages =
    props.messagesData.map(
      m => m.senderMe
        ? <MessageItem key={m.id} message={m.message} avatar={m.avatar} add_class='reverse'/>
        : <MessageItem key={m.id} message={m.message} avatar={m.avatar}/>
    );
